...
| [RuFlo Hive Mind](.github/workflows/ruflo-hivemind.yml) | **Reusable** workflow: headless `hive-mind init` (hierarchical-mesh + Byzantine consensus), then `hive-mind spawn` with a queen-led strategic swarm on the caller repo, then commits and pushes changes (excluding root dot-directories). Requires `ANTHROPIC_API_KEY`. See [example caller](.github/workflows/ruflo-hivemind-example.yml). |